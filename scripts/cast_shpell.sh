...
if [[ "standard" == "${mode}" ]]; then
  ...
  (. ~/.config/tmux/plugins/tmux-grimoire/scripts/shpell.sh "$custom_shpell" "$custom_command" "$replay_flag" &)
elif [[ "ephemeral" == "${mode}" ]]; then
  (. ~/.config/tmux/plugins/tmux-grimoire/scripts/ephemeral_shpell.sh "$custom_shpell" "$custom_command" &)
fi
